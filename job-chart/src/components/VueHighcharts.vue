<template>
  <div>
    <div id="chartbox" style="width: 100%; height: 300px"></div>
  </div>
</template>
<script>
import * as Highcharts from "highcharts";
import mockData from "../assets/wel.json";
...
export default {
  ...
  mounted() {
    this.getSeries().then((series) => {
      const option = {
        chart: { type: "spline" },
        title: { text: "Seek Wellington IT requires" },
        xAxis: {
          type: "datetime",
          labels: {
            formatter() {
  ...
  methods: {
    async getSeries() {
      ...
      // use API
      // const res = await axios.get("http://192.168.68.74:11000/wel");
